...
// QueryLogger 是可选的结构化 SQL 日志能力；未实现时数据库诊断保持静默。
type QueryLogger interface {
	SqlCtx(msg string, ctx map[string]interface{})
}
...
// DialectName 返回当前 SQL 连接使用的稳定方言名称；非 SQL 连接返回空字符串。
func (database *DB) DialectName() string {
	if database == nil {
		return ""
	}
	database.mu.RLock()
	...
	return connectionDialect(managed.connection)
}
...
func Connect(config Config) (*DB, error) {
	...
	database.queryLogging = config.Debug || config.TriggerSQL
...
// Query 执行原生 SQL 查询；传统驱动可继续使用 RawQueryable，
// 但只有 ContextualRawQueryable 能保证默认超时实际传到底层。
...
func (db *DB) Query(sql string, args ...interface{}) ([]map[string]interface{}, error) {
	...
	defer cancel()
	return db.queryContext(ctx, true, sql, args...)
}
...
func (db *DB) QueryContext(ctx context.Context, sql string, args ...interface{}) ([]map[string]interface{}, error) {
	return db.queryContext(ctx, false, sql, args...)
}

// queryContext 只允许无上下文兼容入口回退到传统驱动；显式上下文必须完整传到底层。
func (db *DB) queryContext(ctx context.Context, allowLegacy bool, sql string, args ...interface{}) ([]map[string]interface{}, error) {
	if ctx == nil {
	...
	if db.queryLoggingEnabled() {
		traceContext := map[string]interface{}{
			"sql":       redactSQLText(sql, dialect),
			"arg_count": len(args),
		}
		defer db.traceQueryOperation("query", traceContext)()
	}
	...
	if raw, ok := connection.(RawQueryable); allowLegacy && ok {
	...
	return nil, db.reportError("query", fmt.Errorf("%w: 当前连接不支持可取消的原生查询", ErrUnsupportedFeature), map[string]interface{}{
		"sql":  redactSQLText(sql, dialect),
		"args": redactArgCount(args),
	})
}

// Execute 执行原生 SQL 命令；传统驱动可继续使用 RawQueryable，
// 但只有 ContextualRawQueryable 能保证默认超时实际传到底层。
// 对应 ThinkPHP 的 Db::execute()。
func (db *DB) Execute(sql string, args ...interface{}) (int64, error) {
	ctx, cancel := databaseOperationContext(context.Background())
	defer cancel()
	return db.executeContext(ctx, true, sql, args...)
}

// ExecuteContext 使用显式上下文执行原生命令。
func (db *DB) ExecuteContext(ctx context.Context, sql string, args ...interface{}) (int64, error) {
	return db.executeContext(ctx, false, sql, args...)
}

// executeContext 只允许无上下文兼容入口回退到传统驱动；显式上下文必须完整传到底层。
func (db *DB) executeContext(ctx context.Context, allowLegacy bool, sql string, args ...interface{}) (int64, error) {
	if ctx == nil {
	...
	if db.queryLoggingEnabled() {
		...
		defer db.traceQueryOperation("execute", traceContext)()
	}
	if raw, ok := connection.(ContextualRawQueryable); ok {
	...
	return 0, db.reportError("execute", fmt.Errorf("%w: 当前连接不支持可取消的原生命令", ErrUnsupportedFeature), map[string]interface{}{
		"sql":  redactSQLText(sql, dialect),
		"args": redactArgCount(args),
	})
}
...
// traceQueryOperation 在真实数据库操作周围建立耗时边界；关闭诊断时仅返回空函数。
func (db *DB) traceQueryOperation(operation string, ctx map[string]interface{}) func() {
	if !db.queryLoggingEnabled() {
		return func() {}
	}
	startedAt := time.Now()
	return func() {
		db.reportQuery(operation, time.Since(startedAt), ctx)
	}
}

// queryLoggingEnabled 在构造诊断上下文前读取开关，避免关闭诊断时产生脱敏和 map 分配。
func (db *DB) queryLoggingEnabled() bool {
	if db == nil {
		return false
	}
	db.mu.RLock()
	enabled := db.queryLogging
	db.mu.RUnlock()
	return enabled
}

// reportQuery 只记录脱敏后的查询形状和耗时，不记录任何绑定值。
func (db *DB) reportQuery(operation string, duration time.Duration, ctx map[string]interface{}) {
	if db == nil {
		return
	}
	db.mu.RLock()
	...
	logger := db.logger
	db.mu.RUnlock()
	queryLogger, ok := logger.(QueryLogger)
	if !enabled || !ok || queryLogger == nil {
		return
	}
	...
	queryLogger.SqlCtx("database operation", logContext)
}
