...
func TestModelConfigurationAndConvenienceMethods(t *testing.T) {
	...
	if _, err := invalid.SelectMaps(); !errors.Is(err, ErrInvalidModel) {
...
func TestModelQueryTerminalBusinessFlows(t *testing.T) {
	...
	if _, err := NewModel(model.db, "users").OnlyTrashed().SelectMaps(); !errors.Is(err, ErrInvalidModel) {
	...
	paginator, err := Paginate[typedPageDTO](model.Query(), 2, 1)
	...
	chunkCalls := 0
	if err := Chunk(model.Query(), 1, func(rows []typedPageDTO) (bool, error) {
		chunkCalls++
		return false, nil
	...
	cursorCalls := 0
	if err := ChunkById(model.Query(), 10, func(rows []typedPageDTO) (bool, error) {
		cursorCalls++
		return false, nil
	...
	traceKey := struct{ name string }{"trace"}
	ctx := context.WithValue(context.Background(), traceKey, "ctx-1")
	advancedRows, err := model.newModelQuery().WithContext(ctx).Join("profiles", "users.id = profiles.user_id").SelectMaps()
	_, hasAdvancedDeadline := connection.lastContext.Deadline()
	if err != nil || len(advancedRows) != 2 || connection.lastContext.Value(traceKey) != "ctx-1" || !hasAdvancedDeadline || !strings.Contains(connection.lastRawSQL, "JOIN profiles") {
		t.Fatalf("模型高级查询或上下文传播错误: rows=%#v sql=%q err=%v", advancedRows, connection.lastRawSQL, err)
	}
	if affected, err := model.newModelQuery().WithContext(ctx).WhereField("id", "=", 1).Inc("score", 2).Update(nil); err != nil || affected != 2 || connection.lastContext.Value(traceKey) != "ctx-1" {
...
func TestModelSeekPageAppliesGetters(t *testing.T) {
	...
	first, err := SeekPage[typedPageDTO](model.Query(), 1, nil)
	if err != nil {
	...
	second, err := SeekPage[typedPageDTO](model.Query(), 1, first.NextCursor)
	if err != nil {
...
func TestModelRelationPreloadingCoversAllRelationTypes(t *testing.T) {
	...
	rows, err := users.With("profile", "manager", "roles").SelectMaps()
	if err != nil {
	...
	if _, err := users.With("profile", "profile").SelectMaps(); !errors.Is(err, ErrInvalidRelation) {
		t.Fatalf("重复预加载应返回 ErrInvalidRelation，实际为 %v", err)
	}
	if _, err := users.With("undefined").SelectMaps(); !errors.Is(err, ErrInvalidRelation) {
