...
func (q *Query) quoteSafeClause(clause string) string {
	...
	return quoteConditionIdentifiers(clause, builder.QuoteIdentifier)
}

func quoteConditionIdentifiers(clause string, quote func(string) string) string {
	...
	for _, location := range locations {
		...
		if safeClauseKeywords[lower] || isFunction && allowedExpressionFunctions[lower] {
			...
			result.WriteString(quote(token))
		}
		last = location[1]
	}
...
// traceOperation 为 ORM 终端操作记录脱敏后的查询形状和耗时。
func (q *Query) traceOperation(operation string) func() {
	if q == nil || q.db == nil || !q.db.queryLoggingEnabled() {
		return func() {}
	}
	return q.db.traceQueryOperation(operation, q.logContext())
}
...
func (q *Query) cloneWithCapacity(whereExtra, argsExtra int) *Query {
	...
	cloned := *q
	cloned.predicate = q.predicate.clone()
...
func (q *Query) operationPredicate() (Predicate, error) {
	...
	return q.predicate.clone(), q.predicate.err
}
