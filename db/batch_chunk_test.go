...
func (c *batchRecorderConn) QueryContext(_ context.Context, sql string, args ...interface{}) ([]map[string]interface{}, error) {
...
func (c *batchRecorderConn) ExecuteContext(_ context.Context, sql string, args ...interface{}) (int64, error) {
...
func TestInsertAllSplitsByDialectBindLimit(t *testing.T) {
	cases := []struct {
		...
		{name: "sqlserver-one-column", build: &builder.Sqlsrv{}, fields: 1, want: 1000},
		{name: "sqlserver-two-columns", build: &builder.Sqlsrv{}, fields: 2, want: 1000},
...
// TestSQLServerInsertAllExecutesMultipleBatches 验证列数较少时仍按 VALUES 行上限拆分，并自动开启事务。
func TestSQLServerInsertAllExecutesMultipleBatches(t *testing.T) {
	const rowCount = 1001
	for _, columns := range []int{1, 2} {
		t.Run(fmt.Sprintf("%d列", columns), func(t *testing.T) {
			connection, recorder := newRecordingHardeningSQLConnection(t)
			...
			if _, err := NewDB(connection).Table("records").InsertAll(rows); err != nil {
				t.Fatal(err)
			}
			if recorder.recordedExecCount() != 2 || recorder.recordedBeginCount() != 1 {
				t.Fatalf("没有按行上限在同一事务内拆分: exec=%d begin=%d", recorder.recordedExecCount(), recorder.recordedBeginCount())
			}
		})
	}
}
