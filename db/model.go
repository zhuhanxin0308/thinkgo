...
func (m *Model) SetDB(db *DB) error {
	...
	m.mu.Lock()
	if m.transaction != nil || (m.record != nil && m.record.persisted) {
		m.mu.Unlock()
		return fmt.Errorf("%w: 已绑定事务或记录不能更换数据库", ErrInvalidModel)
	}
	m.db = db
	m.mu.Unlock()
	return nil
...
func (m *Model) BelongsToMany(relatedModel *Model, pivotTable string, foreignKey string, relatedForeignKey string, localKeyValue interface{}) ([]map[string]interface{}, error) {
	...
	pivotRows, err := m.newModelQuery().newRelationTableQuery(database, pivotTable).Field(relatedForeignKey).Where(foreignKey+" = ?", localKeyValue).Select()
	if err != nil {
	...
	// 3. 查询关联模型
	return m.newModelQuery().relationModelQuery(relatedModel).WhereIn(relatedModel.primaryKeyField(), relatedKeys).SelectMaps()
}
...
// Find 查询单条记录到结构体；未命中时返回 false 并保持目标不变。
func (m *Model) Find(target any) (bool, error) {
	return m.newModelQuery().Find(target)
}

// Select 查询多条记录到结构体切片，整批映射成功后才更新目标。
func (m *Model) Select(target any) error {
	return m.newModelQuery().Select(target)
}
...
func (m *Model) Create(v interface{}) error {
	if m.ownsRecord(v) {
		...
		defer r.mu.Unlock()
		if r.persisted {
			return fmt.Errorf("%w: 记录已经创建", ErrInvalidModel)
		}
		if _, err := snapshotModelFields(r.owner); err != nil {
			return err
		}
...
func (m *Model) HasOne(relatedModel *Model, foreignKey string, localKey interface{}) (map[string]interface{}, error) {
	...
	return m.newModelQuery().relationModelQuery(relatedModel).Where(foreignKey+" = ?", localKey).FindMap()
}
...
func (m *Model) HasMany(relatedModel *Model, foreignKey string, localKey interface{}) ([]map[string]interface{}, error) {
	...
	return m.newModelQuery().relationModelQuery(relatedModel).Where(foreignKey+" = ?", localKey).SelectMaps()
}
...
func (m *Model) BelongsTo(relatedModel *Model, foreignKey interface{}, ownerKey string) (map[string]interface{}, error) {
	...
	return m.newModelQuery().relationModelQuery(relatedModel).Where(ownerKey+" = ?", foreignKey).FindMap()
}
...
func (m *Model) structToMap(v interface{}) (map[string]interface{}, error) {
	...
	for _, field := range metadata.fields {
		...
		fieldValue := modelFieldValue(value, field.indexPath, false)
