...
func TestTransactionQueryDuringDatabaseCloseDoesNotDeadlock(t *testing.T) {
	...
	go func() {
		rows, queryErr := transaction.Table("users").Select()
		if queryErr == nil && (len(rows) != 1 || rows[0]["value"] != int64(1)) {
			queryErr = fmt.Errorf("事务查询未返回测试驱动的数据: %#v", rows)
		}
		queryDone <- queryErr
	}()
	select {
	case queryErr := <-queryDone:
		if queryErr != nil {
			t.Fatalf("关闭等待期间已有事务必须能够继续查询: %v", queryErr)
		}
	case <-time.After(time.Second):
