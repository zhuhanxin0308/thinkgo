...
// TestModelExplicitCreateAndUpdate 验证无记录绑定时仍可显式执行结构体创建和更新。
func TestModelExplicitCreateAndUpdate(t *testing.T) {
	connection := &modelPrimaryKeyConnection{
		insertID:     int64(9),
		capabilities: DriverCapabilities{InsertIDKinds: []InsertIDKind{InsertIDInteger}},
	}
	...
	if err := model.Create(created); err != nil || created.ID != 9 {
		t.Fatalf("显式创建结构体失败: value=%#v err=%v", created, err)
	}
	updated := &modelSaveUser{ID: 9, Name: "Grace"}
	if err := model.Update(updated); err != nil {
		t.Fatalf("显式更新结构体失败: %v", err)
	}
	if inserts, updates := connection.insertCalls.Load(), connection.updateCalls.Load(); inserts != 1 || updates != 1 {
		t.Fatalf("每次显式调用必须只执行一次对应写入: inserts=%d updates=%d", inserts, updates)
	}
}

func TestModelUpdateAllowsExistingStringPrimaryKeyWithIntegerIDDriver(t *testing.T) {
	connection := &modelPrimaryKeyConnection{
		insertID:     int64(9),
		capabilities: DriverCapabilities{InsertIDKinds: []InsertIDKind{InsertIDInteger}},
	}
	value := &modelStringKeyUser{ID: "user-9", Name: "Grace"}
	if err := NewModel(NewDB(connection), "users").Update(value); err != nil {
		t.Fatalf("现有字符串业务主键必须能够显式更新: %v", err)
	}
	if inserts, updates := connection.insertCalls.Load(), connection.updateCalls.Load(); inserts != 0 || updates != 1 {
		t.Fatalf("字符串主键更新不得插入记录: inserts=%d updates=%d", inserts, updates)
	}
}
...
func TestInvalidModelDependenciesReturnErrors(t *testing.T) {
	if _, err := NewModel(nil, "users").SelectMaps(); !errors.Is(err, ErrInvalidModel) {
		t.Fatalf("nil DB 模型应返回 ErrInvalidModel，实际为 %v", err)
	}
	if _, err := NewModel(NewDB(&mockConnection{}), "").SelectMaps(); !errors.Is(err, ErrInvalidModel) {
...
func TestRelationLoadingKeepsKeyTypesAndAppliesRelatedGetters(t *testing.T) {
	...
	rows, err := users.With("profiles").SelectMaps()
	if err != nil {
...
func TestRelationLoadingRejectsMissingKeys(t *testing.T) {
	...
	if _, err := users.With("profiles").SelectMaps(); !errors.Is(err, ErrInvalidRelation) {
