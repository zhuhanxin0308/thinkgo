...
// TestMysqlRejectsNonUTCTimezoneParameters 验证直接使用连接器也不能绕过框架 UTC 契约。
func TestMysqlRejectsNonUTCTimezoneParameters(t *testing.T) {
	for _, parameters := range []map[string]string{
		{"loc": "Asia/Shanghai"},
		{"time_zone": "'Asia/Shanghai'"},
	} {
		if _, err := buildMysqlDSN(mysqlConfigWithParams(parameters)); !errors.Is(err, db.ErrInvalidDatabaseConfig) {
			t.Fatalf("非 UTC MySQL 参数应被拒绝: params=%#v err=%v", parameters, err)
		}
	}
}
...
func TestBuildMysqlDSNIncludesHealthCheckParameters(t *testing.T) {
	...
	if !parsed.CheckConnLiveness || !parsed.ParseTime || parsed.Loc != time.UTC || parsed.TLSConfig != "true" {
