...
const sessionDataWindowsShareMode = windows.FILE_SHARE_READ | windows.FILE_SHARE_WRITE | windows.FILE_SHARE_DELETE

// removeSessionFileIfSame 在同一个 Windows 句柄上完成身份复核和删除标记，避免检查后按路径删除。
func removeSessionFileIfSame(path string, expected os.FileInfo) (bool, error) {
	handle, err := openSessionDataFileForRemoval(path)
...
func openSessionDataFileForRemoval(path string) (*os.File, error) {
