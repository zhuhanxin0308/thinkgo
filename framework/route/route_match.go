...
// hasMatchingRouteDomain 判断同一路径索引中是否存在当前主机可用的域名变体。
func hasMatchingRouteDomain(candidates []*Route, host string) bool {
	for _, candidate := range candidates {
		if candidate.domain == "" || candidate.domain == host {
			return true
		}
...
func sortHTTPMethods(methods map[string]bool) []string {
	...
	sort.Slice(result, func(left, right int) bool {
		leftPriority, leftKnown := priority[result[left]]
		rightPriority, rightKnown := priority[result[right]]
