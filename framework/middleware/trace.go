...
func (t *Trace) Handle(req *context.Request, next func(*context.Request) *context.Response) *context.Response {
	...
	if strings.Contains(lowerContent, "</html>") || strings.Contains(lowerContent, "</body>") {
		...
		debugBar := buildDebugBar(info)
		if strings.Contains(lowerContent, "</body>") {
			resp.Content(strings.Replace(strContent, "</body>", debugBar+"</body>", 1))
		} else {
			resp.Content(strContent + debugBar)
		}
	}
