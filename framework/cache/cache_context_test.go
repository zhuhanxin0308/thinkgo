...
func TestCacheContextMethodsPropagateContext(t *testing.T) {
	...
	_, setContext, getManyContext, setManyContext, _, _, _, lockContext := driver.contexts()
	if setContext == nil || setContext.Value(cacheContextKey{}) != "request-1" {
		t.Fatal("SetContext 未收到原始上下文")
	}
	if getManyContext == nil || getManyContext.Value(cacheContextKey{}) != "request-1" {
		t.Fatal("GetManyContext 未收到原始上下文")
	}
	if setManyContext == nil || setManyContext.Value(cacheContextKey{}) != "request-1" {
		t.Fatal("SetManyContext 未收到原始上下文")
	}
...
func TestCacheContextMutationMethodsPropagateContext(t *testing.T) {
	...
	_, _, _, _, hasContext, deleteContext, clearContext, _ := driver.contexts()
