...
func (c *Cache) HasContext(ctx context.Context, key string) (bool, error) {
	...
	if contextual, ok := driver.(ContextualHasser); ok {
		return contextual.HasContext(ctx, key)
	}
...
// RememberWithLock 需要驱动同时支持 DistributedLocker 和 LockRenewer，才能在写回前确认 owner 仍有效。
// 支持续租的驱动会在加载和写入期间自动续租；仅支持 DistributedLocker 的自定义驱动会明确返回 ErrCacheLockUnsupported。
...
func (c *Cache) RememberWithLockContext(ctx context.Context, key string, ttl, lockTTL time.Duration, callback func() (interface{}, error)) (value interface{}, resultErr error) {
	...
	releaseDriver()
	if !lockerSupported || !renewerSupported {
	...
	if resultErr = c.SetContext(ctx, key, value, ttl); resultErr != nil {
		return nil, resultErr
	}
	return value, nil
}
