...
func (q *Query) WhereTime(field string, operator string, values ...interface{}) *Query {
	...
	if start, end, err := buildTimeRange(normalizedOperator, time.Now()); err == nil {
		q.where = append(q.where, fmt.Sprintf("%s BETWEEN ? AND ?", field))
	...
	if normalizedOperator == "between" {
		...
		return q.WhereBetween(field, normalizeTimeValue(values[0]), normalizeTimeValue(values[1]))
	}

	if len(values) != 1 {
		return q.setError(fmt.Errorf("whereTime %s requires exactly 1 value", operator))
	}
	return q.WhereField(field, operator, normalizeTimeValue(values[0]))
}
...
func (q *Query) WhereField(field string, op string, value interface{}) *Query {
	...
	q.where = append(q.where, fmt.Sprintf("%s %s ?", field, op))
...
func (q *Query) WhereMap(conditions map[string]interface{}) *Query {
	for field, value := range conditions {
		if err := validateIdentifier(field); err != nil {
			return q.setError(fmt.Errorf("unsafe whereMap field %q: %w", field, err))
		}
		q.where = append(q.where, fmt.Sprintf("%s = ?", field))
		q.args = append(q.args, value)
	}
	return q
}
...
func (q *Query) WhereNotIn(field string, values []interface{}) *Query {
	...
	q.where = append(q.where, fmt.Sprintf("%s NOT IN (%s)", field, strings.Join(placeholders, ", ")))
	q.args = append(q.args, values...)
	return q
}
...
func (q *Query) WhereNull(field string) *Query {
	...
	q.where = append(q.where, fmt.Sprintf("%s IS NULL", field))
	return q
}
...
func (q *Query) WhereNotNull(field string) *Query {
	...
	q.where = append(q.where, fmt.Sprintf("%s IS NOT NULL", field))
	return q
}
...
func (q *Query) HavingField(field string, op string, value interface{}) *Query {
	...
	if q.having != "" {
		q.having += fmt.Sprintf(" AND %s %s ?", field, op)
	} else {
		q.having = fmt.Sprintf("%s %s ?", field, op)
	}
...
func (q *Query) Inc(field string, step ...int) *Query {
	...
	q.setExprs = append(q.setExprs, fmt.Sprintf("%s = %s + %d", field, field, amount))
	return q
}
