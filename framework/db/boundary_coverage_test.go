...
func TestPaginationValueObjects(t *testing.T) {
	...
	paginator, err := buildPaginator(nil, 5, 2, 2)
	if err != nil || paginator.LastPage != 3 || !paginator.HasMore {
		t.Fatalf("分页对象构造结果错误: paginator=%#v err=%v", paginator, err)
	}
	if offset, err := (&Paginator{Page: 1, PageSize: 20}).Offset(); err != nil || offset != 0 {
		t.Fatalf("第一页偏移量错误: offset=%d err=%v", offset, err)
	}
	if _, err := buildPaginator(nil, -1, 1, 20); !errors.Is(err, ErrInvalidPagination) {
...
func TestPredicateAndCursorValidationContracts(t *testing.T) {
	...
	if _, _, err := raw.compileNonSQL(); !errors.Is(err, ErrUnsafeExpression) {
...
func TestQueryBudgetAndChunkLimitContracts(t *testing.T) {
	...
	if _, err := buildPaginator(nil, math.MaxInt64, maxInt, maxInt); err == nil {
...
func TestDatabaseMetadataAndPartialWriteContracts(t *testing.T) {
	...
	if nilConnection.Location() != time.Local {
		t.Fatalf("空 SQL 连接应安全回退本地时区: %v", nilConnection.Location())
	}
	connection := &SQLConnection{}
	connection.SetLocation(nil)
	if connection.Location() != time.Local {
		t.Fatalf("空时区应回退本地时区: %v", connection.Location())
	}
...
// TestNeo4jMetadataAndValueNormalizationContracts 验证 Neo4j 时间值和嵌套属性的时区归一化。
func TestNeo4jMetadataAndValueNormalizationContracts(t *testing.T) {
	connection := &Neo4jConnection{}
	caps := connection.Capabilities()
	if !caps.MatchedCountKnown || caps.ModifiedCountKnown || len(caps.InsertIDKinds) != 3 {
		t.Fatalf("Neo4j 能力声明错误: %#v", caps)
	}
	if normalizeNeo4jMapInLocation(nil, time.UTC) != nil {
		t.Fatal("空 Neo4j 属性应保持 nil")
	}
	location := time.FixedZone("test", 8*60*60)
	value := time.Date(2026, time.July, 29, 10, 0, 0, 0, time.UTC)
	normalized := normalizeNeo4jMapInLocation(map[string]interface{}{
		"created_at": value,
		"nested":     map[string]interface{}{"items": []interface{}{value}},
	}, location)
	createdAt, ok := normalized["created_at"].(time.Time)
	if !ok || createdAt.Location() != location || createdAt.Hour() != 18 {
		t.Fatalf("Neo4j 时间未按应用时区归一化: %#v", normalized["created_at"])
	}
	nested := normalized["nested"].(map[string]interface{})["items"].([]interface{})
	if nested[0].(time.Time).Location() != location {
		t.Fatalf("嵌套 Neo4j 时间未归一化: %#v", nested)
	}
}
