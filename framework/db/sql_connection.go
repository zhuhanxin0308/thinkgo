...
func NewSQLConnection(database *sql.DB, build Builder) *SQLConnection {
	return &SQLConnection{
		...
		statements:        make(map[string]*sql.Stmt, defaultStatementCacheCapacity),
	}
}
...
func (c *SQLConnection) queryRowsContext(ctx context.Context, query string, args ...interface{}) (*sql.Rows, error) {
	...
	// 锁覆盖从缓存取语句到真正开始执行，避免语句在使用前被并发淘汰关闭。
	c.statementMu.RLock()
	statement := c.statements[query]
	if statement != nil {
		rows, err := statement.QueryContext(ctx, args...)
		c.statementMu.RUnlock()
		return rows, err
	}
	c.statementMu.RUnlock()
	c.statementMu.Lock()
	if statement = c.statements[query]; statement == nil {
		var err error
		statement, err = c.cachedStatementLocked(ctx, query)
		if err != nil {
...
func (c *SQLConnection) execContext(ctx context.Context, query string, args ...interface{}) (sql.Result, error) {
	...
	// ExecContext 必须与语句淘汰处于同一临界区，保证语句至少能安全进入 database/sql。
	c.statementMu.RLock()
	statement := c.statements[query]
	if statement != nil {
...
// cachedStatement 获取或创建有界缓存中的语句。
// 语句按首次进入顺序淘汰，避免动态 SQL 无限占用连接内存。
func (c *SQLConnection) cachedStatementLocked(ctx context.Context, query string) (*sql.Stmt, error) {
	if statement, ok := c.statements[query]; ok {
...
// closeStatements 关闭连接级缓存中的所有语句，并清空缓存索引。
func (c *SQLConnection) closeStatements() error {
	if c == nil {
		return nil
	}
	c.statementMu.Lock()
	statements := make([]*sql.Stmt, 0, len(c.statements))
	...
	for _, statement := range statements {
		closeErr = errors.Join(closeErr, statement.Close())
	}
	return closeErr
}
