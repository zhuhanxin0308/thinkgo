...
func TestBuiltInCommandDefinitions(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.signature, func(t *testing.T) {
			...
			} else if len(arguments) != 1 || arguments[0].Name != testCase.argumentName || arguments[0].Required != testCase.required {
...
func TestVersionAndListCommandsWriteExpectedOutput(t *testing.T) {
	...
	if strings.TrimSpace(stdout.String()) != "ThinkGo Framework v2.0.0" {
