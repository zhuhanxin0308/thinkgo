...
func TestApplicationURLHelpersNormalizeAndValidateInputs(t *testing.T) {
	...
	if err := configuration.Set("app.server.domain", "https://example.com/"); err != nil {
		t.Fatalf("写入服务器域名配置失败: %v", err)
	}
	app := &App{DebugMode: true, config: configuration}
	if !app.IsDebug() {
...
// TestInitializeAppliesUnifiedEnvironmentConfig 验证环境层按完整配置路径统一覆盖应用与 Provider 配置。
func TestInitializeAppliesUnifiedEnvironmentConfig(t *testing.T) {
	basePath := t.TempDir()
	writeTestAppConfigFiles(t, basePath)
	overriddenCachePath := filepath.ToSlash(filepath.Join(basePath, "runtime", "environment-cache"))
	environmentContent := strings.Join([]string{
		"[APP]",
		"APP_DEBUG = true",
		"SERVER_PORT = 19090",
		"",
		"[CACHE]",
		"STORES_FILE_PATH = " + overriddenCachePath,
	}, "\n")
	if err := os.WriteFile(filepath.Join(basePath, ".env"), []byte(environmentContent), 0o600); err != nil {
		t.Fatalf("写入统一环境配置失败: %v", err)
	}
	...
	if !app.DebugMode {
		t.Fatal("APP_APP_DEBUG 未覆盖 app.app_debug")
	}
	if got := app.config.GetInt("app.server.port"); got != 19090 {
		t.Fatalf("APP_SERVER_PORT 未覆盖 app.server.port，实际为 %d", got)
	}
	if got := filepath.Clean(app.config.GetString("cache.stores.file.path")); got != filepath.Clean(overriddenCachePath) {
		t.Fatalf("CACHE_STORES_FILE_PATH 未覆盖缓存目录，实际为 %q", got)
	}
	if _, err := os.Stat(overriddenCachePath); err != nil {
		t.Fatalf("文件缓存 Provider 未使用环境层覆盖后的目录: %v", err)
	}
}

// TestEnvironmentExampleMatchesDeclaredConfigStructure 验证环境模板中的每个值都能按配置默认类型完成合并。
func TestEnvironmentExampleMatchesDeclaredConfigStructure(t *testing.T) {
	...
	if err := os.WriteFile(environmentPath, example, 0o600); err != nil {
		t.Fatalf("创建隔离环境文件失败: %v", err)
	}
	if err := environment.Load(environmentPath); err != nil {
		t.Fatalf("加载环境变量模板失败: %v", err)
	}
	if err := configuration.ApplyEnvironment(environment); err != nil {
		t.Fatalf("环境变量模板与配置结构不兼容: %v", err)
	}
	if host := configuration.GetString("app.server.host"); host != "127.0.0.1" {
		t.Fatalf("APP_SERVER_HOST 未覆盖声明的监听地址: %q", host)
	}
	if port := configuration.GetInt("app.server.port"); port != 39091 {
		t.Fatalf("APP_SERVER_PORT 未按整数类型覆盖声明端口: %d", port)
	}
	if publicPath := configuration.GetString("app.public_path"); publicPath != "web" {
		t.Fatalf("APP_PUBLIC_PATH 未覆盖声明的公共目录: %q", publicPath)
	}
}
...
func TestCreateAppCacheRejectsInvalidConfiguration(t *testing.T) {
	...
	testCases := map[string]map[string]interface{}{
		...
		"expire 为负数": {
			"default": "file",
			"stores": map[string]interface{}{
				"file": map[string]interface{}{"type": "file", "path": "./runtime/cache", "expire": float64(-1)},
			},
		},
		"相对路径越界": {
			"default": "file",
...
func TestInitializeDoesNotFallbackFromInvalidCacheConfig(t *testing.T) {
  ...
  "default": "file",
  "stores": {
    "file": {"type": "file", "path": "./runtime/cache", "expire": -1}
  }
}`
...
func TestInitializeRejectsInvalidBooleanEnvironmentOverride(t *testing.T) {
	basePath := t.TempDir()
	writeTestAppConfigFiles(t, basePath)
	t.Setenv("APP_APP_DEBUG", "not-a-boolean")

	app, _ := initializeTestConsoleApp(t, basePath)
	if err := app.StartupError(); err == nil || !strings.Contains(err.Error(), "APP_APP_DEBUG") {
		t.Fatalf("非法 APP_APP_DEBUG 应进入启动错误，实际为 %v", err)
	}
}
...
func TestEnvironmentUsesUnifiedEnvironmentPrecedence(t *testing.T) {
	previous, existed := os.LookupEnv("APP_APP_ENV")
	if err := os.Unsetenv("APP_APP_ENV"); err != nil {
		t.Fatalf("清理 APP_APP_ENV 失败: %v", err)
	}
	t.Cleanup(func() {
		if existed {
			_ = os.Setenv("APP_APP_ENV", previous)
		} else {
			_ = os.Unsetenv("APP_APP_ENV")
		}
	})

	basePath := t.TempDir()
	writeTestAppConfigFiles(t, basePath)
	if err := os.WriteFile(filepath.Join(basePath, ".env"), []byte("[APP]\nAPP_ENV=staging\n"), 0o600); err != nil {
...
// TestInitializeIncompleteApplicationReturnsStableStartupError 验证不完整应用返回可诊断且稳定的启动错误。
func TestInitializeIncompleteApplicationReturnsStableStartupError(t *testing.T) {
	app := &App{}
	firstErr := app.Initialize()
	if firstErr == nil || errors.Is(firstErr, ErrApplicationInitializationPanic) {
		t.Fatalf("不完整应用应返回普通初始化错误而非依赖 panic，实际为 %v", firstErr)
	}
	if err := app.Initialize(); err == nil || err.Error() != firstErr.Error() {
		t.Fatalf("重复 Initialize 应返回首次初始化结果，首次为 %v，实际为 %v", firstErr, err)
	}
}
...
func TestInitializeBindsControllersAsFactory(t *testing.T) {
	...
	if err := app.RegisterController(controllerName, &perRequestTestController{}); err != nil {
		t.Fatalf("注册测试控制器失败: %v", err)
	}
	if err := app.Initialize(); err != nil {
		t.Fatalf("初始化显式注册控制器的应用失败: %v", err)
	}
	defer func() {
		if err := app.Close(); err != nil {
			t.Errorf("关闭控制器工厂测试应用失败: %v", err)
		}
	}()
...
func TestInitializeLoadsUnixTimestampValueType(t *testing.T) {
  ...
  "connections": {
	...
	if app.db.Location().String() != defaultTimezoneName {
		t.Fatalf("数据库未继承应用时区: got=%q", app.db.Location())
	}
