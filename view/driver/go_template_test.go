...
// TestGoTemplatePerRenderFunctionsAreIsolated 验证单次请求函数只覆盖本次克隆模板，
// 不会污染共享函数表、缓存模板或后续请求。
func TestGoTemplatePerRenderFunctionsAreIsolated(t *testing.T) {
	directory := t.TempDir()
	if err := os.WriteFile(filepath.Join(directory, "index.html"), []byte(`{{ value }}`), 0o600); err != nil {
		t.Fatalf("写入模板失败: %v", err)
	}
	driver := NewGoTemplate()
	...
	if err := driver.SetFuncMap(map[string]interface{}{"value": func() string { return "default" }}); err != nil {
		t.Fatalf("设置共享模板函数失败: %v", err)
	}
	requestContent, err := driver.FetchWithFuncMap("index", nil, map[string]interface{}{
		"value": func() string { return "request" },
	})
	if err != nil || requestContent != "request" {
		t.Fatalf("单次模板函数覆盖失败: content=%q err=%v", requestContent, err)
	}
	defaultContent, err := driver.Fetch("index", nil)
	if err != nil || defaultContent != "default" {
		t.Fatalf("单次覆盖不得污染共享模板: content=%q err=%v", defaultContent, err)
	}
	if _, err := driver.FetchWithFuncMap("index", nil, map[string]interface{}{"value": 1}); !errors.Is(err, ErrInvalidTemplateFunction) {
		t.Fatalf("非法单次函数必须被拒绝，实际错误为 %v", err)
	}
}

// TestGoTemplateDisabledCacheObservesFileChanges 验证显式关闭缓存后每次渲染都会读取
// 当前模板内容，且不会悄悄积累缓存条目。
func TestGoTemplateDisabledCacheObservesFileChanges(t *testing.T) {
	directory := t.TempDir()
	indexPath := filepath.Join(directory, "index.html")
	if err := os.WriteFile(indexPath, []byte(`first`), 0o600); err != nil {
		t.Fatalf("写入初始模板失败: %v", err)
	}
	driver := NewGoTemplate()
	if err := driver.Config(map[string]interface{}{"view_path": directory, "cache": false}); err != nil {
		t.Fatalf("配置无缓存模板驱动失败: %v", err)
	}
	first, err := driver.Fetch("index", nil)
	if err != nil || first != "first" {
		t.Fatalf("首次无缓存渲染失败: content=%q err=%v", first, err)
	}
	if err := os.WriteFile(indexPath, []byte(`second`), 0o600); err != nil {
		t.Fatalf("更新模板失败: %v", err)
	}
	second, err := driver.Fetch("index", nil)
	if err != nil || second != "second" {
		t.Fatalf("无缓存渲染必须读取最新内容: content=%q err=%v", second, err)
	}
	driver.mutex.RLock()
	cacheEntries := len(driver.cache)
	driver.mutex.RUnlock()
	if cacheEntries != 0 {
		t.Fatalf("关闭缓存后不得保留模板条目，实际为 %d", cacheEntries)
	}
}
...
// TestGoTemplateCacheIsBounded 验证缓存达到容量上限后会淘汰旧条目并保留新解析模板，
// 而不是让所有后续模板永久绕过缓存形成稳定的性能退化。
func TestGoTemplateCacheIsBounded(t *testing.T) {
	directory := t.TempDir()
	indexPath := filepath.Join(directory, "index.html")
	if err := os.WriteFile(indexPath, []byte(`bounded`), 0o600); err != nil {
		t.Fatalf("写入模板失败: %v", err)
	}
	driver := NewGoTemplate()
	if err := driver.Config(map[string]interface{}{"view_path": directory}); err != nil {
	...
	syntheticTemplate := template.Must(template.New("synthetic").Parse("cached"))
	driver.mutex.Lock()
	for index := 0; index < maxCachedTemplates; index++ {
		driver.storeTemplateLocked(
			filepath.Join(directory, "synthetic-"+strconv.Itoa(index)+".html"),
			syntheticTemplate,
		)
	}
	driver.mutex.Unlock()
	if content, err := driver.Fetch("synthetic-0", nil); err != nil || content != "cached" {
		t.Fatalf("命中旧模板以更新最近使用顺序失败: content=%q err=%v", content, err)
	}
	...
	cachedIndex := driver.cache[indexPath]
	retainedRecent := driver.cache[filepath.Join(directory, "synthetic-0.html")]
	evictedOldest := driver.cache[filepath.Join(directory, "synthetic-1.html")]
	driver.mutex.RUnlock()
	if entries != maxCachedTemplates {
		t.Fatalf("模板缓存不得超过上限，实际为 %d", entries)
	}
	if cachedIndex == nil {
		t.Fatal("缓存达到上限后新解析模板仍必须进入缓存")
	}
	if retainedRecent == nil || evictedOldest != nil {
		t.Fatal("LRU 淘汰必须保留刚命中的模板并移除最久未使用模板")
	}
	if err := os.Remove(indexPath); err != nil {
		t.Fatalf("删除源模板失败: %v", err)
	}
	cachedContent, err := driver.Fetch("index", nil)
	if err != nil || cachedContent != "bounded" {
		t.Fatalf("新模板必须由替换后的缓存继续提供: content=%q err=%v", cachedContent, err)
	}
}
