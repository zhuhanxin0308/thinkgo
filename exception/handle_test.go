...
func TestRenderBusinessExceptionUsesConfiguredStatus(t *testing.T) {
	...
	reported := NewBusinessException(1001, "库存不足").WithStatus(http.StatusConflict)

	handler.Report(reported)
	if err := handler.Render(recorder, req, reported); err != nil {
...
func TestRenderHttpExceptionClientErrorSkipsErrorStack(t *testing.T) {
	...
	req.Header.Set("Accept", "application/json")
	recorder := httptest.NewRecorder()
	reported := NewHttpException(http.StatusNotFound, "not found")
...
func TestKnownExceptionLogsEscapeControlCharacters(t *testing.T) {
	...
	req.Header.Set("Accept", "application/json")
	recorder := httptest.NewRecorder()
	reported := NewHttpException(http.StatusNotFound, "missing\r\ninjected")
	handler.Report(reported)
	if err := handler.Render(recorder, req, reported); err != nil {
		t.Fatalf("渲染 HTTP 异常失败: %v", err)
	}
