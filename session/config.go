...
var (
	...
	// ErrSessionCommitted 表示 HTTP 最终响应边界已越过，后续变更无法可靠持久化或写 Cookie。
	ErrSessionCommitted = errors.New("Session 响应已经提交")
...
func ParseConfig(raw map[string]interface{}) (Config, error) {
	allowed := map[string]bool{
		"name": true, "var_session_id": true, "type": true, "store": true, "prefix": true,
		"storage_path": true, "redis": true, "cookie_path": true,
	...
	if value, exists := raw["var_session_id"]; exists {
		config.VarSessionID, err = sessionConfigString(value, "var_session_id")
		if err != nil {
			return Config{}, err
		}
	}
	...
	if value, exists := raw["store"]; exists && value != nil {
		config.Store, err = sessionConfigString(value, "store")
		if err != nil {
			return Config{}, err
		}
	}
	if value, exists := raw["prefix"]; exists {
		config.Prefix, err = sessionConfigString(value, "prefix")
		if err != nil {
			return Config{}, err
		}
	}
...
func validateSessionConfig(config Config) error {
	...
	if config.DriverType != "file" && config.DriverType != "cache" && config.DriverType != "memory" && config.DriverType != "redis" {
		return invalidSessionConfig("type", "仅支持 file、cache、memory 或 redis")
	}
	if len(config.VarSessionID) > maxSessionKeyBytes || hasSessionControl(config.VarSessionID) {
		return invalidSessionConfig("var_session_id", "过长或包含控制字符")
	}
	if len(config.Store) > maxSessionKeyBytes || hasSessionControl(config.Store) {
		return invalidSessionConfig("store", "过长或包含控制字符")
	}
	if len(config.Prefix) > maxSessionKeyBytes || hasSessionControl(config.Prefix) {
		return invalidSessionConfig("prefix", "过长或包含控制字符")
	}
