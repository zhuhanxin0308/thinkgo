...
func TestFileCacheCounterPreservesExpiryAndRejectsInvalidValues(t *testing.T) {
	...
	if item.Expiry.Location() != time.UTC {
		t.Fatalf("持久化缓存过期时间必须使用 UTC: %v", item.Expiry.Location())
	}
	now := time.Now()
...
// TestFileCachePersistsLockExpiryInUTC 验证跨进程锁载荷不会携带部署机器时区。
func TestFileCachePersistsLockExpiryInUTC(t *testing.T) {
	driver, _ := newTestFileDriver(t)
	if acquired, err := driver.AcquireLock("utc-lock", "owner", time.Minute); err != nil || !acquired {
		t.Fatalf("获取 UTC 持久化锁失败: acquired=%t err=%v", acquired, err)
	}
	data, err := os.ReadFile(driver.lockFilePath("utc-lock"))
	if err != nil {
		t.Fatalf("读取锁载荷失败: %v", err)
	}
	var payload fileLockPayload
	if err := json.Unmarshal(data, &payload); err != nil {
		t.Fatalf("解析锁载荷失败: %v", err)
	}
	if payload.Expiry.Location() != time.UTC {
		t.Fatalf("持久化锁过期时间必须使用 UTC: %v", payload.Expiry.Location())
	}
	if released, err := driver.ReleaseLock("utc-lock", "owner"); err != nil || !released {
		t.Fatalf("释放 UTC 持久化锁失败: released=%t err=%v", released, err)
	}
}
...
func TestFileCacheCounterHonorsCrossProcessLock(t *testing.T) {
	...
	go func() {
		guardDone <- driver.withMutationGuard("counter", func() error {
			close(started)
			<-release
			return nil
		})
