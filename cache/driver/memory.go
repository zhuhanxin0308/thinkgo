...
const (
	...
	cacheFenceMetadataPrefix      = contract.FenceMetadataPrefix
)
...
func (c *Memory) Clear() error {
	...
	c.lock.Lock()
	preserved := make(map[string]item)
	for key, stored := range c.items {
		if isCapacityExemptKey(key) {
			stored.order = nil
...
func (c *Memory) storeItemLocked(key string, value interface{}, expiry time.Time, version uint64, protected bool) error {
	...
	if isCapacityExemptKey(key) {
		stored := item{value: value, expiry: expiry, version: version, protected: true}
		stored.order = c.order.PushBack(key)
		c.items[key] = stored
		return nil
	}
	if protected {
		if c.capacityUsedLocked() >= c.maxEntries {
			return ErrMemoryCapacityExhausted
		}
	} else {
		for c.capacityUsedLocked() >= c.maxEntries {
...
func (c *Memory) capacityUsedLocked() int {
	used := 0
...
func isMetadataKey(key string) bool {
	return strings.HasPrefix(key, cacheTagMetadataPrefix) || strings.HasPrefix(key, cacheTagReverseMetadataPrefix) || strings.HasPrefix(key, cacheFenceMetadataPrefix)
}

func isCapacityExemptKey(key string) bool {
	return strings.HasPrefix(key, cacheFenceMetadataPrefix)
}
