...
func (c *File) Get(key string) (value interface{}, found bool, result error) {
	if c == nil || c.path == "" {
		return nil, false, ErrInvalidCachePath
	}
	c.operationMu.RLock()
	defer c.operationMu.RUnlock()
	result = c.withMutationGuard(key, func() error {
		lock := c.keyLock(key)
		lock.Lock()
		defer lock.Unlock()
		item, exists, err := c.readItemLocked(key, time.Now())
		found = exists
		if err != nil || !found {
			return err
		}
...
func (c *File) Set(key string, value interface{}, ttl time.Duration) error {
	...
	c.operationMu.RLock()
	defer c.operationMu.RUnlock()
	return c.withMutationGuard(key, func() error {
		lock := c.keyLock(key)
		lock.Lock()
		defer lock.Unlock()
...
func (c *File) Clear() error {
	...
	return c.withAllMutationGuards(context.Background(), c.clearLocked)
}

func (c *File) clearLocked() error {
	...
	for _, entry := range entries {
		...
		if remove {
			...
			if isManagedCacheFilename(name) {
				preserve, preserveErr := c.shouldPreserveFenceEntry(path)
				if preserveErr != nil {
...
func (c *File) shouldPreserveFenceEntry(path string) (bool, error) {
	...
	if err = json.Unmarshal(data, &item); err != nil || len(item.Value) == 0 {
		return false, fmt.Errorf("%w: %v", ErrCorruptCacheEntry, err)
	}
	return strings.HasPrefix(item.Key, cacheFenceMetadataPrefix), nil
}
...
func (c *File) changeCounter(key string, step int64, subtract bool) (value int64, resultErr error) {
	...
	c.operationMu.RLock()
	defer c.operationMu.RUnlock()
	resultErr = c.withMutationGuard(key, func() error {
		lock := c.keyLock(key)
		lock.Lock()
		defer lock.Unlock()
...
func (c *File) RenewLock(key string, owner string, ttl time.Duration) (bool, error) {
	...
	data, err := json.Marshal(fileLockPayload{Owner: owner, Expiry: now.Add(ttl).UTC()})
	if err != nil {
...
func (c *File) readItemLocked(key string, now time.Time) (*storedItem, bool, error) {
	...
	if item.Key != "" && item.Key != key {
		return nil, false, fmt.Errorf("%w: 文件键与请求键不匹配", ErrCorruptCacheEntry)
	}
...
func (c *File) writeItemLocked(key string, value interface{}, expiry time.Time) error {
	...
	data, err := json.Marshal(storedItem{Key: key, Value: encodedValue, Expiry: expiry})
	if err != nil {
		return err
	}
...
func (c *File) readManagedFile(path string, limit int64) ([]byte, os.FileInfo, bool, error) {
	return c.readManagedFileWithOpen(path, limit, openCacheDataFile)
}
...
func writeCacheLockPayload(handle *os.File, payload fileLockPayload) error {
	...
	payload.Expiry = payload.Expiry.UTC()
