...
type wrappedCacheBuilder struct{ db.Builder }
type oracleCacheBuilder struct{ builder.Pgsql }

func (*oracleCacheBuilder) DialectName() string { return "oracle" }

// TestCacheSchemaUsesDialectCapability 验证 Builder 包装器不会改变方言识别和建表语法。
func TestCacheSchemaUsesDialectCapability(t *testing.T) {
	for _, base := range []db.Builder{&builder.Sqlsrv{}, &oracleCacheBuilder{}, &builder.Pgsql{}} {
		direct, err := NewDB(&db.SQLConnection{Builder: base}, "think_cache")
		if err != nil {
			t.Fatal(err)
		}
		wrapped, err := NewDB(&db.SQLConnection{Builder: &wrappedCacheBuilder{Builder: base}}, "think_cache")
		if err != nil {
			t.Fatal(err)
		}
		if direct.createTableSQL() != wrapped.createTableSQL() {
			t.Fatalf("包装器改变了 %s 方言语法: %s", base.DialectName(), wrapped.createTableSQL())
		}
		if base.DialectName() == "oracle" && !strings.Contains(wrapped.createTableSQL(), "EXCEPTION WHEN OTHERS") {
			t.Fatal("Oracle 建表缺少已存在处理")
		}
	}
}
...
func TestDBCacheBackendDataValidation(t *testing.T) {
	...
	invalidValueKey := mustStorageKey(t, driver, "invalid-value")
	conn.rows[invalidValueKey] = map[string]interface{}{
		"key": invalidValueKey, "value": 123, "expiry": int64(0),
	}
	...
	invalidExpiryKey := mustStorageKey(t, driver, "invalid-expiry")
	conn.rows[invalidExpiryKey] = map[string]interface{}{
		"key": invalidExpiryKey, "value": `"value"`, "expiry": "invalid",
	}
	...
	negativeExpiryKey := mustStorageKey(t, driver, "negative-expiry")
	conn.rows[negativeExpiryKey] = map[string]interface{}{
		"key": negativeExpiryKey, "value": `"value"`, "expiry": int64(-1),
	}
	...
	legacyKey := mustStorageKey(t, driver, "legacy-expired")
	conn.rows[legacyKey] = map[string]interface{}{
		"key": legacyKey, "value": []byte(`"value"`), "expiry": time.Now().Add(-time.Second).Unix(),
	}
	if _, found, getErr := driver.Get("legacy-expired"); getErr != nil || found {
		t.Fatalf("旧秒级过期值应兼容并删除: found=%t err=%v", found, getErr)
	}
	if _, exists := conn.rows[legacyKey]; exists {
...
func TestDBExpiredReadDoesNotDeleteConcurrentRefresh(t *testing.T) {
	...
	physicalKey := mustStorageKey(t, driver, "race")
	conn.rows[physicalKey] = map[string]interface{}{
		"key": physicalKey, "value": `"expired"`, "expiry": time.Now().Add(-time.Second).UnixMilli(),
	}
...
func (c *recordingCacheConn) Update(_ context.Context, request db.UpdateRequest) (db.UpdateResult, error) {
	...
	if c.reportUnchanged && reflect.DeepEqual(row, updatedRow) {
	...
	for _, clause := range predicate.Clauses() {
		...
		case "<=", ">", ">=", "<":
			if field == "key" {
				...
				if operator == "<" && comparison >= 0 || operator == "<=" && comparison > 0 || operator == ">" && comparison <= 0 || operator == ">=" && comparison < 0 {
					return false
				}
				continue
			}
...
func mustStorageKey(t *testing.T, driver *DB, key string) string {
	t.Helper()
	physicalKey, err := driver.storageKey(key)
	if err != nil {
		t.Fatal(err)
	}
	return physicalKey
