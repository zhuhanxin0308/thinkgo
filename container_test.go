...
// TestContainerScopedLifecycle 验证 Scoped 在同一请求作用域内复用、跨作用域隔离，且不能从根容器误解析。
func TestContainerScopedLifecycle(t *testing.T) {
	container := NewContainer()
	var builds atomic.Int32
	container.BindScoped("request_service", func() interface{} {
		return &struct{ sequence int32 }{sequence: builds.Add(1)}
	})
	if _, err := container.Make("request_service"); !errors.Is(err, ErrContainerScopeRequired) {
		t.Fatalf("根容器解析 Scoped 应返回 ErrContainerScopeRequired，实际为 %v", err)
	}
	...
	first, err := firstScope.Make("request_service")
	if err != nil {
		t.Fatalf("首次作用域解析失败: %v", err)
	}
	again, err := firstScope.Make("request_service")
	if err != nil || first != again {
		t.Fatalf("同一作用域没有复用实例: first=%p again=%p err=%v", first, again, err)
	}
	second, err := secondScope.Make("request_service")
	if err != nil || second == first {
		t.Fatalf("不同作用域没有隔离实例: first=%p second=%p err=%v", first, second, err)
	}
	if builds.Load() != 2 {
		t.Fatalf("Scoped 工厂构建次数错误: %d", builds.Load())
	}
}

// TestContainerScopedFactoryResolvesScopedDependency 验证工厂注入的容器保持当前作用域。
func TestContainerScopedFactoryResolvesScopedDependency(t *testing.T) {
	container := NewContainer()
	container.BindScoped("dependency", func() interface{} { return &struct{}{} })
	container.BindScoped("service", func(current *Container) (interface{}, error) {
		return current.Make("dependency")
	})
	scope := container.NewScope()
	defer func() { _ = scope.Close() }()
	dependency, err := scope.Make("dependency")
	if err != nil {
		t.Fatalf("解析作用域依赖失败: %v", err)
	}
	service, err := scope.Make("service")
	if err != nil || service != dependency {
		t.Fatalf("作用域工厂没有复用当前依赖: dependency=%p service=%p err=%v", dependency, service, err)
	}
}

// TestContainerRejectsScopedDependencyCapturedBySingleton 验证单例不能直接或经由 Factory 捕获请求作用域实例。
func TestContainerRejectsScopedDependencyCapturedBySingleton(t *testing.T) {
	container := NewContainer()
	container.BindScoped("request_dependency", func() interface{} { return &struct{}{} })
	container.BindFactory("factory_bridge", func(current *Container) (interface{}, error) {
		return current.Make("request_dependency")
	})
	container.Bind("singleton", func(current *Container) (interface{}, error) {
		return current.Make("factory_bridge")
	})
	scope := container.NewScope()
	defer func() { _ = scope.Close() }()
	if _, err := scope.Make("singleton"); !errors.Is(err, ErrScopedDependencyCaptured) {
		t.Fatalf("单例捕获 Scoped 服务必须被拒绝，实际为 %v", err)
	}
}
