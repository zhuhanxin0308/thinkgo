...
func TestRouterOptionalParamAndNamedURL(t *testing.T) {
	...
	if builtURL != "/users/18/profile.html?keyword=go" {
...
func TestRouterExtensionParticipatesInMatchingAndURLGeneration(t *testing.T) {
	...
	if err = registered.WithPattern("name", `.+`); err != nil {
		t.Fatalf("设置允许编码斜杠的显式变量规则失败: %v", err)
	}
...
func TestRouterCompleteMatchControlsDynamicRouteSuffix(t *testing.T) {
	...
	if err := completeRouter.EnableAutoRoute(false); err != nil {
		t.Fatalf("关闭自动路由失败: %v", err)
	}
	if err := completeRouter.SetCompleteMatch(true); err != nil {
		t.Fatalf("设置完整匹配规则失败: %v", err)
	}
...
func TestRouterRejectsDuplicateAndInvalidDefinitions(t *testing.T) {
	...
	var nilStandardHandler http.HandlerFunc
	if _, err = router.Get("/typed-nil", nilStandardHandler); !errors.Is(err, ErrInvalidRouteHandler) {
		t.Fatalf("类型化空标准库处理器必须被拒绝，实际为 %v", err)
	}
...
func TestRouterMethodSemantics(t *testing.T) {
	...
	if headRoute == nil || !headRoute.IsAuto() || headRoute.Handler() != "items/index" {
		t.Fatalf("HEAD 未命中显式规则时应继续 URL 调度，实际为 %#v", headRoute)
	}

	postRoute, _, err := router.Match(fwcontext.MustNewRequest(
		httptest.NewRequest(http.MethodPost, "http://example.com/items", nil),
	))
	if err != nil || postRoute == nil || !postRoute.IsAuto() || postRoute.Handler() != "items/index" {
		t.Fatalf("POST 未命中显式规则时应继续 URL 调度，route=%#v err=%v", postRoute, err)
	}
	...
	if response.GetStatus() != http.StatusNoContent || response.Headers().Get("Allow") != "GET, POST, PUT, DELETE" {
		t.Fatalf("自动 OPTIONS 响应错误: status=%d allow=%q", response.GetStatus(), response.Headers().Get("Allow"))
	}
}

// TestAutomaticOptionsDoesNotInheritRouteMiddleware 验证 ThinkPHP 自动 OPTIONS
...
func TestAutomaticOptionsDoesNotInheritRouteMiddleware(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("创建 GET CORS 中间件失败: %v", err)
	}
	postCors, err := middleware.NewCors(middleware.CorsConfig{
		AllowOrigins: []string{"https://post.example"},
		AllowMethods: []string{http.MethodPost},
	})
	if err != nil {
		t.Fatalf("创建 POST CORS 中间件失败: %v", err)
	}
	if _, err = router.Get("/items/:id", "Item@Show", getCors); err != nil {
		t.Fatalf("注册 GET 路由失败: %v", err)
	}
	if err = router.Group("/items", func(group *Group) error {
		_, registerErr := group.Post("/:id", "Item@Update")
		return registerErr
	}, postCors); err != nil {
		t.Fatalf("注册带分组 CORS 的 POST 路由失败: %v", err)
	}

	raw := httptest.NewRequest(http.MethodOptions, "http://example.com/items/42", nil)
	raw.Header.Set("Origin", "https://post.example")
	...
	if err != nil || matched == nil {
		t.Fatalf("自动预检匹配失败: route=%#v params=%#v err=%v", matched, params, err)
	}
	if params != nil || matched.HasMiddleware() {
		t.Fatalf("自动 OPTIONS 不应继承动态参数或目标路由中间件: params=%#v route=%#v", params, matched)
	}
	handler, ok := matched.Handler().(func(*fwcontext.Request) *fwcontext.Response)
	if !ok {
		t.Fatalf("自动 OPTIONS 处理器类型错误: %T", matched.Handler())
	}
	response := handler(request)
	if response.GetStatus() != http.StatusNoContent || response.GetHeader("Allow") != "GET, POST, PUT, DELETE" {
		t.Fatalf("自动 OPTIONS 响应错误: status=%d headers=%#v", response.GetStatus(), response.Headers())
	}
	if response.GetHeader("Access-Control-Allow-Origin") != "" {
		t.Fatalf("自动 OPTIONS 不应复制 POST 路由的 CORS 策略: %#v", response.Headers())
	}
}
...
func TestResourceOnlyAndExcept(t *testing.T) {
	...
	if matched, _, matchErr := router.Match(fwcontext.MustNewRequest(httptest.NewRequest(http.MethodPost, "http://example.com/posts", nil))); matched != nil || matchErr != nil {
		t.Fatalf("Only 未保留的方法在强制路由下应表现为未找到，route=%#v err=%v", matched, matchErr)
	}
