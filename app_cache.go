...
func createAppCache(app *App, rawConfig map[string]interface{}) (*cache.Cache, error) {
	...
	optionsByName := make(map[string]cache.StoreOptions, len(specs))
	for _, spec := range specs {
		optionsByName[spec.name] = cache.StoreOptions{Expire: spec.expire, TagPrefix: spec.tagPrefix}
	}
	if err = manager.ConfigureStore("default", optionsByName[defaultStore]); err != nil {
		return nil, errors.Join(fmt.Errorf("配置默认缓存 store %q 失败: %w", defaultStore, err), manager.Close())
	}
	...
	for _, name := range names {
		...
		if err = manager.ConfigureStore(name, optionsByName[name]); err != nil {
			return nil, errors.Join(
				fmt.Errorf("配置缓存 store %q 失败: %w", name, err),
				manager.Close(),
			)
		}
	}
...
func parseAppCacheStore(resolvePath func(string) (string, error), name string, storeConfig map[string]interface{}) (appCacheStoreSpec, error) {
	...
	spec := appCacheStoreSpec{name: name, driverType: driverType, tagPrefix: "tag:"}
	if rawPrefix, exists := storeConfig["prefix"]; exists {
		prefix, ok := rawPrefix.(string)
		if !ok || !utf8.ValidString(prefix) || containsAppStoragePathControl(prefix) || len(prefix) > 1024 {
			return appCacheStoreSpec{}, fmt.Errorf("缓存 store %q 的 prefix 必须是安全字符串", name)
		}
		spec.prefix = prefix
	}
	if rawExpire, exists := storeConfig["expire"]; exists {
		expire, err := readConfigIntValue(rawExpire)
		if err != nil {
			return appCacheStoreSpec{}, fmt.Errorf("缓存 store %q 的 expire 必须是非负整数秒: %w", name, err)
		}
		spec.expire = time.Duration(expire) * time.Second
	}
	if rawTagPrefix, exists := storeConfig["tag_prefix"]; exists {
		tagPrefix, ok := rawTagPrefix.(string)
		if !ok || tagPrefix == "" || !utf8.ValidString(tagPrefix) || containsAppStoragePathControl(tagPrefix) || len(tagPrefix) > 128 {
			return appCacheStoreSpec{}, fmt.Errorf("缓存 store %q 的 tag_prefix 必须是安全的非空字符串", name)
		}
		spec.tagPrefix = tagPrefix
	}
	if rawSerialize, exists := storeConfig["serialize"]; exists {
		serialize, ok := rawSerialize.([]interface{})
		if !ok || len(serialize) != 0 {
			return appCacheStoreSpec{}, fmt.Errorf("缓存 store %q 的 serialize 仅支持默认空列表；Go 自定义序列化应通过驱动实现", name)
		}
	}
	switch driverType {
	case "memory":
		if err := rejectUnknownCacheStoreFields(name, storeConfig, "type", "max_entries", "prefix", "expire", "tag_prefix", "serialize"); err != nil {
	...
	case "file":
		if err := rejectUnknownCacheStoreFields(name, storeConfig, "type", "path", "prefix", "expire", "tag_prefix", "serialize"); err != nil {
			return appCacheStoreSpec{}, err
		}
		rawPath, pathExists := storeConfig["path"]
		pathValue, pathValid := rawPath.(string)
		if !pathExists || !pathValid {
			return appCacheStoreSpec{}, fmt.Errorf("文件缓存 store %q 的 path 必须是字符串", name)
		}
		if pathValue == "" {
			pathValue = "cache"
		}
	...
	case "redis":
		if err := rejectUnknownCacheStoreFields(
			name,
			storeConfig,
			"type", "host", "port", "password", "select", "timeout_ms", "prefix", "expire", "tag_prefix", "serialize", "allow_flush_db",
		...
		for key, value := range storeConfig {
			if key != "type" && key != "expire" && key != "tag_prefix" && key != "serialize" {
...
func createAppCacheDriver(spec appCacheStoreSpec) (cache.Driver, error) {
	...
	switch spec.driverType {
	case "memory":
		driver, err = cacheDriver.NewMemoryWithMaxEntries(spec.maxMemoryEntries)
	case "file":
		driver, err = cacheDriver.NewFile(spec.path)
	case "redis":
		driver, err = redisDriver.NewRedis(spec.redisConfig)
	default:
	...
	namespacePrefix := spec.prefix
	if spec.driverType == "redis" {
		// Redis 驱动自身用 prefix 约束 SCAN/Clear，不能在外层重复添加。
		namespacePrefix = ""
	}
	return cache.NewNamespaceDriver(driver, namespacePrefix, spec.tagPrefix)
}
