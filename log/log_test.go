...
type blockingRecordingDriver struct {
	entered chan struct{}
	release chan struct{}
	once    sync.Once
	mu      sync.Mutex
...
func (d *blockingRecordingDriver) SaveEntries(entries []*LogEntry) error {
	d.once.Do(func() { close(d.entered) })
	<-d.release
	d.mu.Lock()
	d.saved += len(entries)
...
func (d *blockingRecordingDriver) WriteEntry(*LogEntry) error {
	d.mu.Lock()
	d.written++
	d.mu.Unlock()
	return nil
}
func (*blockingRecordingDriver) Close() error { return nil }

func (d *blockingRecordingDriver) savedCount() int {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.saved
}

func (d *blockingRecordingDriver) writeCount() int {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.written
}
...
// TestLogCloseDrainsEntriesAfterCanceledFlush 验证显式 Flush 的调用方超时只结束等待，
// 不会丢弃已经接受或随后入队的日志；关停仍会完整排空并保存全部条目。
func TestLogCloseDrainsEntriesAfterCanceledFlush(t *testing.T) {
	driver := &blockingRecordingDriver{
		entered: make(chan struct{}),
		release: make(chan struct{}),
	}
	logger := NewLog(driver)
	if err := logger.SetBufferSize(8); err != nil {
		t.Fatalf("设置可靠性测试日志缓冲区失败: %v", err)
	}
	if err := logger.SetFlushInterval(time.Hour); err != nil {
		t.Fatalf("设置可靠性测试刷新周期失败: %v", err)
	}
	logger.Info("first")
	logger.Info("second")
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Millisecond)
	...
	if err := logger.Flush(ctx); !errors.Is(err, context.DeadlineExceeded) {
		close(driver.release)
		t.Fatalf("阻塞 Flush 应按调用方截止时间返回，实际为 %v", err)
	}
	select {
	case <-driver.entered:
	default:
	...
	close(driver.release)
	if err := logger.Close(); err != nil {
		t.Fatalf("取消 Flush 后关停日志器失败: %v", err)
	}
	if saved := driver.savedCount(); saved != 3 {
		t.Fatalf("关停必须保存取消 Flush 前后的全部日志，实际为 %d", saved)
	}
}
