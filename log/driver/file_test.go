...
func TestFileDriverDefaultsToUTC(t *testing.T) {
	driver := NewFile(t.TempDir())
	if driver.location != time.UTC || driver.now().Location() != time.UTC {
		t.Fatalf("文件日志默认时区必须为 UTC: location=%v now=%v", driver.location, driver.now().Location())
	}
	driver.SetLocation(nil)
	if driver.location != time.UTC {
		t.Fatalf("nil 文件日志时区必须回退 UTC: %v", driver.location)
	}
}
