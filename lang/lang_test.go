...
// TestLangMergeAllOverlaysAtomically 验证多应用语言包会覆盖同名全局翻译、
// 保留全局独有翻译，并在应用语言目录损坏时保留完整旧快照。
func TestLangMergeAllOverlaysAtomically(t *testing.T) {
	globalDirectory := t.TempDir()
	applicationDirectory := t.TempDir()
	if err := os.WriteFile(filepath.Join(globalDirectory, "zh-cn.json"), []byte(`{"shared":"global","global_only":"global"}`), 0o600); err != nil {
		t.Fatalf("写入全局语言包失败: %v", err)
	}
	applicationFile := filepath.Join(applicationDirectory, "zh-cn.json")
	if err := os.WriteFile(applicationFile, []byte(`{"shared":"application","application_only":"application"}`), 0o600); err != nil {
		t.Fatalf("写入应用语言包失败: %v", err)
	}
	manager := NewLang()
	if err := manager.LoadAll(globalDirectory); err != nil {
		t.Fatalf("加载全局语言包失败: %v", err)
	}
	if err := manager.MergeAll(applicationDirectory); err != nil {
		t.Fatalf("叠加应用语言包失败: %v", err)
	}
	if got := manager.Get("shared", nil, "zh-cn"); got != "application" {
		t.Fatalf("应用翻译没有覆盖全局翻译: %q", got)
	}
	if got := manager.Get("global_only", nil, "zh-cn"); got != "global" {
		t.Fatalf("全局独有翻译没有保留: %q", got)
	}
	if got := manager.Get("application_only", nil, "zh-cn"); got != "application" {
		t.Fatalf("应用独有翻译没有加载: %q", got)
	}

	if err := os.WriteFile(applicationFile, []byte(`{"shared":`), 0o600); err != nil {
		t.Fatalf("写入损坏应用语言包失败: %v", err)
	}
	if err := manager.MergeAll(applicationDirectory); err == nil {
		t.Fatal("损坏应用语言包必须返回错误")
	}
	if got := manager.Get("shared", nil, "zh-cn"); got != "application" {
		t.Fatalf("叠加失败后不应破坏既有语言快照: %q", got)
	}
}
