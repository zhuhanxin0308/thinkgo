...
// TestOperationalRoutesEnforceRestrictedSourceNetworks 验证运维端点只接受显式 CIDR 白名单中的直连来源。
func TestOperationalRoutesEnforceRestrictedSourceNetworks(t *testing.T) {
	zeroPolicyApp := &App{health: health.NewRegistry()}
	zeroPolicyRequest := fwcontext.MustNewRequest(httptest.NewRequest(http.MethodGet, "http://localhost"+OperationalLivenessPath, nil))
	zeroPolicyRequest.Raw().RemoteAddr = "127.0.0.1:42100"
	if response := zeroPolicyApp.livenessRouteWithHealth(zeroPolicyApp.health, zeroPolicyRequest); response.GetStatus() != http.StatusNotFound {
		t.Fatalf("未初始化访问策略必须失败关闭: status=%d", response.GetStatus())
	}

	app := &App{
		health:                     health.NewRegistry(),
		operationalAccess:          OperationalAccessRestricted,
		operationalAllowedPrefixes: []netip.Prefix{netip.MustParsePrefix("127.0.0.0/8")},
	}
	allowed := fwcontext.MustNewRequest(httptest.NewRequest(http.MethodGet, "http://localhost"+OperationalLivenessPath, nil))
	allowed.Raw().RemoteAddr = "127.0.0.25:42100"
	if response := app.livenessRouteWithHealth(app.health, allowed); response.GetStatus() != http.StatusOK {
		t.Fatalf("白名单来源应允许访问存活端点: status=%d", response.GetStatus())
	}

	denied := fwcontext.MustNewRequest(httptest.NewRequest(http.MethodGet, "http://localhost"+OperationalLivenessPath, nil))
	denied.Raw().RemoteAddr = "203.0.113.10:42100"
	if response := app.livenessRouteWithHealth(app.health, denied); response.GetStatus() != http.StatusNotFound {
		t.Fatalf("非白名单来源应隐藏运维端点: status=%d", response.GetStatus())
	}
}
...
func TestOperationalRoutesUseSafeFrameworkHandlers(t *testing.T) {
	app := &App{
		...
		operationalAccess: OperationalAccessPublic,
	}
