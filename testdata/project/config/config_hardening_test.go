...
func TestConfigLoadRejectsUnsafeFilesAndJSONShapes(t *testing.T) {
	t.Run("writable by group", func(t *testing.T) {
		...
		file := filepath.Join(t.TempDir(), "writable.json")
		if err := os.WriteFile(file, []byte(`{"value":true}`), 0o664); err != nil {
			t.Fatalf("写入权限测试配置失败: %v", err)
		}
		if err := os.Chmod(file, 0o664); err != nil {
			t.Fatalf("设置权限测试配置失败: %v", err)
		}
		if err := NewConfig().Load(file, "app"); !errors.Is(err, ErrConfigPermissionsTooOpen) {
			t.Fatalf("可被组改写的配置必须被拒绝: %v", err)
		}
	})
