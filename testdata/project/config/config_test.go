...
// TestConfigCloneAndReplaceUseDeepSnapshots 验证项目配置基线与应用工作副本
// 之间不存在嵌套 map、slice 或旧命名空间的共享引用。
func TestConfigCloneAndReplaceUseDeepSnapshots(t *testing.T) {
	source := NewConfig()
	if err := source.Set("app.features", map[string]interface{}{
		"names": []interface{}{"alpha", map[string]interface{}{"enabled": true}},
	}); err != nil {
		t.Fatalf("写入源配置失败: %v", err)
	}
	snapshot := source.Clone()
	if err := source.Set("app.features.names", []interface{}{"changed"}); err != nil {
		t.Fatalf("修改源配置失败: %v", err)
	}
	if got := snapshot.Get("app.features.names").([]interface{})[0]; got != "alpha" {
		t.Fatalf("配置克隆仍与源配置共享数据: %#v", got)
	}

	target := NewConfig()
	if err := target.Set("stale.value", true); err != nil {
		t.Fatalf("写入目标旧配置失败: %v", err)
	}
	target.ReplaceWithSnapshot(snapshot)
	if target.Has("stale") {
		t.Fatal("替换快照后不应保留目标的旧命名空间")
	}
	if err := snapshot.Set("app.features.names", []interface{}{"snapshot-changed"}); err != nil {
		t.Fatalf("修改快照失败: %v", err)
	}
	if got := target.Get("app.features.names").([]interface{})[0]; got != "alpha" {
		t.Fatalf("目标配置仍与输入快照共享数据: %#v", got)
	}
}
