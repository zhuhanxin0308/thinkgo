...
func TestRequestMetadataAccessors(t *testing.T) {
	...
	if req.Pathinfo() != "files/report.json" || req.Ext() != "json" {
	...
	if req.BaseUrl() != "/files/report.json" || req.BaseUrl(true) != "https://example.com:8443/files/report.json" || req.Root() != "" || req.Root(true) != "https://example.com:8443" {
		t.Fatalf("ThinkPHP URL 辅助结果错误: base=%q complete_base=%q root=%q complete_root=%q", req.BaseUrl(), req.BaseUrl(true), req.Root(), req.Root(true))
	}
	...
	if req.Scheme() != "https" || req.Port() != 8443 || req.Cookie("sid") != "abc" || req.Cookie("missing") != "" {
		t.Fatalf("协议、端口或 Cookie 错误: scheme=%q port=%d", req.Scheme(), req.Port())
	}
	if req.Header("X-Empty", "fallback") != "" || req.Header("X-Missing", "fallback") != "fallback" {
		t.Fatal("请求头显式空值与缺失值语义错误")
	}
	if req.ContentType() != "" || req.Server("X-Requested-With") != "XMLHttpRequest" {
		t.Fatal("ContentType 默认值或 Server 兼容映射错误")
	}
...
func TestRequestScalarConversions(t *testing.T) {
	...
	req.SetRoute("too_large", uint64(math.MaxUint64))
	req.SetRoute("fraction", 1.5)
...
func TestRequestZeroValueAndNilRaw(t *testing.T) {
	var zero Request
	zero.Set("key", "value")
	zero.SetRoute("route", "value")
	if zero.GetData("key") != "value" || zero.Middleware("key") != "value" || zero.Input("key") != "" || zero.Input("route") != "value" {
		t.Fatal("零值 Request 的数据读写错误")
	}
	if zero.Method() != http.MethodGet || zero.Host() != "" || zero.Path() != "" || zero.Raw() != nil {
		t.Fatal("零值 Request 应使用 ThinkPHP 的 GET 默认方法，其它元数据为空")
	}
