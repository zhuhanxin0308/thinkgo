...
func TestRequestRouteParametersUseIsolatedNamespace(t *testing.T) {
	...
	if req.Route("_session") != "route-value" || req.Param("_session") != "route-value" {
		t.Fatal("不存在同名 query/body 时，路由参数应进入 Route/Param")
	}
}

// TestRequestParamPrefersBodyOverQuery 验证 ThinkPHP Param 的 body、query、route 合并优先级。
func TestRequestParamPrefersBodyOverQuery(t *testing.T) {
	...
	req.SetRoute("role", "owner")
	if value := req.Param("role"); value != "user" {
		t.Fatalf("body 参数应继续覆盖同名路由参数，实际为 %q", value)
	}
}
...
func TestRequestTypedAccessorsRejectLossyValues(t *testing.T) {
	...
	req.SetRoute("fraction", 1.25)
	req.SetRoute("overflow", uint64(math.MaxUint64))
	req.SetRoute("nan", math.NaN())
	req.SetRoute("numeric_bool", 2)
	req.SetRoute("panic_stringer", panickingStringer{})
...
func TestRequestAbsoluteURLsRejectInvalidHost(t *testing.T) {
	for _, host := range []string{
		...
		raw.Host = host
		req := newRequestForTest(t, raw)
		if req.Root(true) != "" || req.BaseUrl(true) != "" || req.Url(true) != "" || req.Port() != 0 {
			t.Fatalf("非法 Host %q 不得生成绝对地址或端口，root=%q base=%q url=%q port=%d", host, req.Root(true), req.BaseUrl(true), req.Url(true), req.Port())
		}
	}
}
...
// TestResponseContentMatchesThinkPHPDefaultHeaders 验证基础响应默认使用 ThinkPHP 的 HTML 类型。
func TestResponseContentMatchesThinkPHPDefaultHeaders(t *testing.T) {
	response := NewResponse().Content("plain")
	headers := response.Headers()
	if headers.Get("Content-Type") != "text/html; charset=utf-8" {
		t.Fatalf("基础响应 Content-Type 错误: %q", headers.Get("Content-Type"))
	}
	if headers.Get("X-Content-Type-Options") != "" {
		t.Fatalf("默认响应不得添加 ThinkPHP 不存在的响应头: %q", headers.Get("X-Content-Type-Options"))
	}
