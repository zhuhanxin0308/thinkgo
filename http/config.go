...
const defaultRequestEndTimeout = 2 * time.Second

const defaultMaxOutstandingRequests = 2048
...
type serverConf struct {
	...
	RequestEndTimeout      time.Duration
...
func parseHTTPConfig(app *framework.App) (serverConf, compressionConf, error) {
	...
	return parseHTTPApplicationConfig(configuration.Get("app", map[string]interface{}{}))
}

// parseProjectHTTPConfig 使用业务应用配置叠加前的项目快照解析宿主监听配置，
// 防止任一 app/<name>/config 修改整个进程的地址、端口或 TLS 协议。
func parseProjectHTTPConfig(app *framework.App) (serverConf, compressionConf, error) {
	if app == nil {
		return serverConf{}, compressionConf{}, fmt.Errorf("%w: 应用不能为空", ErrInvalidHTTPConfig)
	}
	return parseHTTPApplicationConfig(app.ProjectApplicationConfig())
}

func parseHTTPApplicationConfig(raw interface{}) (serverConf, compressionConf, error) {
	applicationValues, err := strictConfigMap(raw, "app")
	if err != nil {
		return serverConf{}, compressionConf{}, err
	}
	serverValues, err := strictConfigMap(applicationValues["server"], "app.server")
	if err != nil {
		return serverConf{}, compressionConf{}, err
	}
	if err = rejectUnknownConfigKeys(serverValues, "app.server", map[string]bool{
		"domain": true, "host": true, "port": true, "allowed_hosts": true, "trusted_proxies": true,
		...
		"request_end_timeout_ms":   true,
		"max_outstanding_requests": true,
		"max_header_bytes":         true, "max_body_bytes": true, "multipart_max_memory_mb": true,
	}); err != nil {
		return serverConf{}, compressionConf{}, err
	}
	server, err := parseServerConfig(serverValues)
	if err != nil {
		return serverConf{}, compressionConf{}, err
	}
	compressionValues, err := strictConfigMap(applicationValues["compression"], "app.compression")
	if err != nil {
		return serverConf{}, compressionConf{}, err
	}
...
func parseServerConfig(values map[string]interface{}) (serverConf, error) {
	...
	if config.RequestEndTimeout, err = configDuration(values, "request_end_timeout_ms", config.RequestEndTimeout); err != nil {
		return serverConf{}, err
	}
	maximumRequests, err := configInteger(values, "max_outstanding_requests", int64(config.MaxOutstandingRequests), 1, 1<<20)
	if err != nil {
		return serverConf{}, err
	}
	config.MaxOutstandingRequests = int(maximumRequests)
