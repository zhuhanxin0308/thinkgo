...
func (c *invalidSignatureController) Bad() (string, string) {
	return "first", "second"
}
...
func TestDispatchCachesAutoRoutePlan(t *testing.T) {
	...
	autoHandler, ok := matchedRoute.Handler().(string)
	if !ok {
		t.Fatalf("自动路由处理器应使用控制器/操作字符串，实际为 %T", matchedRoute.Handler())
	}
	firstPlan := handler.dispatchPlans[autoHandler]
	if firstPlan == nil {
	...
	if secondPlan := handler.dispatchPlans[autoHandler]; secondPlan != firstPlan {
