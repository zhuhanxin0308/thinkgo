...
func (c *hardeningCommand) Execute(input *Input, output *Output) error {
	c.executed = true
	if input != nil {
...
// TestConsoleRunContextPropagatesCancellation 验证嵌入式宿主传入的执行上下文
// 会原样到达命令，而旧 Run 入口仍提供非空背景上下文。
func TestConsoleRunContextPropagatesCancellation(t *testing.T) {
	cli := NewConsole(nil)
	if err := cli.SetOutput(NewOutputWithWriters(&bytes.Buffer{}, &bytes.Buffer{}, false)); err != nil {
		t.Fatalf("设置上下文测试输出失败: %v", err)
	}
	command := &hardeningCommand{name: "context"}
	if err := cli.Register(command); err != nil {
		t.Fatalf("注册上下文测试命令失败: %v", err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := cli.RunContext(ctx, "context"); err != nil {
		t.Fatalf("命令自行处理取消上下文时 Console 不应改写结果: %v", err)
	}
	if command.context != ctx || !errors.Is(command.context.Err(), context.Canceled) {
		t.Fatalf("命令未收到原始取消上下文: %#v", command.context)
	}
	command.context = nil
	if err := cli.Run("context"); err != nil {
		t.Fatalf("兼容 Run 入口执行失败: %v", err)
	}
	if command.context == nil || command.context.Err() != nil {
		t.Fatalf("兼容 Run 入口必须提供可用背景上下文: %#v", command.context)
	}
	var nilContext context.Context
	if err := cli.RunContext(nilContext, "context"); !errors.Is(err, ErrInvalidInput) {
		t.Fatalf("nil 执行上下文必须返回 ErrInvalidInput，实际为 %v", err)
	}
}
