...
func NewInput(args ...string) *Input {
	return NewInputContext(context.Background(), args...)
}

// NewInputContext 创建携带宿主取消、截止时间和值的命令输入。
// nil 上下文只供兼容直接构造场景使用，Context 会将其规范为背景上下文。
func NewInputContext(ctx context.Context, args ...string) *Input {
	return &Input{
...
// Context 返回本次命令执行上下文；旧代码直接构造 Input 时仍保证非空。
func (i *Input) Context() context.Context {
	if i == nil || i.execution == nil {
...
func isCLIIdentifier(value string, short bool) bool {
	...
	for index, current := range runes {
		if unicode.IsLetter(current) || index > 0 && (unicode.IsDigit(current) || current == '-' || current == '_') || short && unicode.IsDigit(current) {
			continue
		}
		return false
	}
